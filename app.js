...
timerId = setInterval(moveBall, 20); // lower second parameter for faster ball movement

// check collisions of ball
function checkCollisions() {
    ...
    for (let i = 0; i < blocks.length; i++) {
        if (
            ...
            // check for win
            if (blocks.length === 0) {
                scoreDisplay.innerHTML = 'YOU WIN' + "<br>" + ' Score: ' + score;
    ...
    if (ballCurrentPosition[1] <= 0) {
        clearInterval(timerId);
        scoreDisplay.innerHTML = 'You lost!'+ "<br>" + ' Score: ' + score;
        document.removeEventListener('keydown', moveUser);
    }
}
